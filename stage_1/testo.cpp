...
int main(){
    ...
    input.modiFile(NUM_OF_LAYERS); //为了强行把一层变成两层而保持芯片面积不变，每层强行变化一下所有坐标缩小了√2倍
	input.divideCircuit(NUM_OF_LAYERS, IS_LOAD1);//强行把一维的只有xy坐标的点分成两层，加上z坐标
    input.generateAP(IS_LOAD2);//调用AP.cpp 生成activity pattern。以上函数都不用管它，不用改
    ...
    input.NNS();//最近邻选择算法，用这种算法每次选最小cost的配对，生成拓扑二叉树

    gettimeofday(&end2,NULL); 
    cout<<"end2_tv_sec:"<<end2.tv_sec<<endl;
    cout<<"end2_tv_usec:"<<end2.tv_usec<<endl;
    diff2= (end2.tv_sec-start2.tv_sec)*1000000+end2.tv_usec-start2.tv_usec;
    ...
    diff1= (end1.tv_sec-start1.tv_sec)*1000000+end1.tv_usec-start1.tv_usec;
