...
int **CAP::loadAP() {
    ...
    for ( int i = 0; i < num_of_sinks; i++ )
        for ( int j = 0; j < INST_LENGTH; j++ ) AP[i][j] = 0;  // initialize AP
...
void CAP::generateModules( bool is_write_file ) {
    ...
    if ( is_write_file )
        f_M << "Ave. Modules: " << ( ave_modules / num_of_inst ) << endl;
    f_M.close();
}
...
void CAP::generateInstStream( bool is_write_file ) {
    ...
    for ( int i = 0; i < INST_LENGTH; i++ ) {
        if ( rand() % HIGH_INST_NUM < 1 ) {  // each(high and low inst.) appear
                                             // 50% of the time in the stream
...
void CAP::generateAP( bool is_write_file ) {
    ...
    if ( is_write_file ) {
        ...
        if ( !f_AP.is_open() )
            cout << "can't create Activity_Pattern file" << endl;
