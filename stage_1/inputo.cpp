...
void CInput::setInputData( string file_path, int benchmark ) {
    ...
    if ( benchmark == ISPD ) {
        file >> chip_size.ll_x >> chip_size.ll_y >> chip_size.ur_x >>
            chip_size.ur_y;
        file >> buf >> source.source_name >> source.x >> source.y >>
            source.buf_name;
        ...
        for ( int i = 0; i < num_of_buflib; i++ ) {
            file >> buflib[i].id >> buflib[i].spice_file >> buflib[i].is_inv >>
                buflib[i].in_cl >> buflib[i].out_cl >> buflib[i].out_rl;
        }
        ...
        for ( int i = 0; i < num_of_blockage; i++ ) {
            block[i].id = i;
            file >> block[i].ll_x >> block[i].ll_y >> block[i].ur_x >>
                block[i].ur_y;
        }
...
void CInput::modiFile( int num_of_layers ) {
    ...
    f_mf.precision( 10 );
    f_mf << chip_size.ll_x << " " << chip_size.ll_y << " "
         << floor( chip_size.ur_x / sqrt( num_of_layers ) ) << " "
         << floor( chip_size.ur_y / sqrt( num_of_layers ) ) << endl;
    f_mf << "source 0 0 0 0" << endl;
    f_mf << "num sink " << num_of_sinks << endl;
    for ( int i = 0; i < num_of_sinks; i++ ) {
        f_mf << i + 1 << " " << floor( sink[i].x / sqrt( num_of_layers ) )
             << " " << floor( sink[i].y / sqrt( num_of_layers ) ) << " "
             << sink[i].cl << endl;
    }
    ...
    for ( int i = 0; i < num_of_buflib; i++ ) {
        f_mf << i << " clkinv" << i << ".subckt " << buflib[i].is_inv << " "
             << buflib[i].in_cl << " " << buflib[i].out_cl << " "
             << buflib[i].out_rl << endl;
    }
    ...
    for ( int i = 0; i < num_of_blockage; i++ ) {
        f_mf << floor( block[i].ll_x / sqrt( num_of_layers ) ) << " "
             << floor( block[i].ll_y / sqrt( num_of_layers ) ) << " "
             << floor( block[i].ur_x / sqrt( num_of_layers ) ) << " "
             << floor( block[i].ur_y / sqrt( num_of_layers ) ) << endl;
    }
}
...
void CInput::NNS() {
    ...
    for ( int cycle_index = 0; cycle_index < num_of_sinks - 1; cycle_index++ ) {
        ...
        tree_node[min_a].parent = tree_node[min_b].parent =
            inner;  // construct init topo tree
        tree_node[inner].data = inner;

        list<MergingCell>::iterator t_mc_it;
        for ( mc_it = mc.begin(); mc_it != mc.end(); ) {
            t_mc_it = mc_it++;
            if ( ( ( *t_mc_it ).a == min_a ) || ( ( *t_mc_it ).a == min_b ) ||
                 ( ( *t_mc_it ).b == min_a ) || ( ( *t_mc_it ).b == min_b ) ) {
                // mc.remove(*mc_it);
                mc.erase( t_mc_it );
            }
        }
        ...
        for ( r_it = remain.begin(); r_it != remain.end(); ) {
            t_r_it = r_it++;
            if ( *t_r_it == min_a || *t_r_it == min_b ) remain.erase( t_r_it );
        }
        // remain.remove(min_a);
        // remain.remove(min_b);
...
double CInput::getMergingCost( int a, int b ) {
    ...
    double BI = ( sink[a].cl * sink[a].p + sink[b].cl * sink[b].p ) +
                0.5 * cg * ( sink[a].ptr + sink[b].ptr );
    // return cw*distance(a.x,a.y,b.x,b.y) + cv*abs(a.z-b.z) +
    // (a.cl*a.p+b.cl*b.p) + cg*(a.ptr+b.ptr);
    // return (GI + LI + BI + ALFA_CL*(sink[a].cl+sink[b].cl)*abs(sink[a].level
    // + sink[b].level));
    // return GI;
...
void CInput::mergeNode( int inner, int a, int b ) {
    ...
    // cout << "id: " << inner <<" " << *t_ap << endl << "p: " << sink[inner].p
    // <<" ptr: " << sink[inner].ptr << endl << endl;
    sink[inner].level++;
}
...
void CInput::DLE() {
    ...
    for ( int i = num_of_sinks; i < 2 * num_of_sinks - 1; i++ ) {
        tree_node[i].left_TSV =
            abs( tree_node[i].l - tree_node[tree_node[i].lchild].l );
        tree_node[i].right_TSV =
            abs( tree_node[i].l - tree_node[tree_node[i].rchild].l );
    }
}
...
void CInput::outputInsertedTopoTree() {
    ...
    for ( int i = 0; i < 2 * num_of_sinks - 1; i++ ) {
        ...
        f_stree << "lchild: " << tree_node[i].lchild
                << "    rchild: " << tree_node[i].rchild << endl;
        f_stree << "left_TSV: " << tree_node[i].left_TSV
                << "     right_TSV: " << tree_node[i].right_TSV << endl;
...
void CInput::output4DME() {
    ...
    f_DME << "This is a output file of the first stage topo tree generation, "
             "and it is the input file of the second stage 3D-DB-DME."
          << endl;
    // f_DME << "chipsize(ll_x, ll_y, ur_x, ur_y), wirelib(r0, c0),
    // buflib(is_inv, in_cl, out_cl, out_rl) " << endl;
    f_DME << "chipsize(ll_x, ll_y, ur_x, ur_y), wirelib(r0, c0), "
             "buf(input_cap), TSV(cap) "
          << endl;
    f_DME << chip_size.ll_x << " " << chip_size.ll_y << " " << chip_size.ur_x
          << " " << chip_size.ur_y << " ";
    f_DME << rw << " " << cw << " ";
    // f_DME << buflib[0].is_inv << " " << buflib[0].in_cl << " " <<
    // buflib[0].out_cl << " " << buflib[0].out_rl << endl;
    f_DME << cg << " " << cv << endl;
    f_DME << "id, x,y,z, Cl, p,ptr, parent, lchild, rchild, left_TSV, right_TSV"
          << endl;
    f_DME << "num_of_nodes: " << 2 * num_of_sinks - 1 << endl;
    for ( int i = 0; i < 2 * num_of_sinks - 1; i++ ) {
        f_DME << sink[i].id << " " << sink[i].x << " " << sink[i].y << " "
              << sink[i].z << " " << sink[i].cl << " " << sink[i].p << " "
              << sink[i].ptr << " " << tree_node[i].parent << " "
              << tree_node[i].lchild << " " << tree_node[i].rchild << " "
