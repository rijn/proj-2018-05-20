...
struct cmpRemove{
    bool operator (/*int re_a, int re_b*/) (const MergingCell & mc) {
       // if ( (mc.a == re_a ) || (mc.a == re_b ) || (mc.b == re_a) || (mc.b == re_b)){
            return true;
       // }
       // else return false;
    };
...
class CInput{
    public:
        CInput() : num_of_sinks(0), num_of_wirelib(0), num_of_buflib(0), num_of_blockage(0){}
        int bench;              // benchmark
        ...
        void divideCircuit(int num_of_layers, bool is_load);      // divide circuit to generate z coordinate
