...
class CGBDME {
   ...
   private:
    ...
    void updateInfo(
        int l,
        int r );  // after merging process, updata info. for the next mergement

    void bufInsertion();  // insert buffers and gates
    // void sourceInsertion();                 // insert source to the root of
    // tree
    ...
    double MD( double x1, double y1, double x2,
               double y2 );  // calc Manhattn Distance of two points
    ...
    bool isColine( double x1, double y1, double x2, double y2, double x3,
                   double y3 );
    bool isIntersection( double x1, double y1, double x2, double y2, double x3,
