...
void CGBDME::setInputData( string file_path ) {
    ...
    getline( file, buf );  // get redundant info.
    file >> chip_size.ll_x >> chip_size.ll_y >> chip_size.ur_x >>
        chip_size.ur_y;
    file >> r0 >> c0 >> cg >> cv;
    rg  = 61.2;
    cgo = 80;
    ...
    for ( int i = 0; i < num_of_nodes; i++ ) {
        file >> node[i].id >> node[i].x >> node[i].y >> node[i].z >>
            node[i].cl >> node[i].p >> node[i].ptr >> node[i].parent >>
            node[i].lchild >> node[i].rchild >> node[i].left_TSV >>
            node[i].right_TSV;
        node[i].t = 0;
    ...
    for ( int i = 0; i < num_of_nodes - 1; i++ ) {
        if ( node[node[i].parent].lchild == i )
            ...
            node[i].nv = node[node[i].parent].right_TSV;
...
void CGBDME::dme( int v_sp ) {
    ...
    f_el << "debug for function calcEdgeLength(l, r) in Bottom-Up Phase"
         << endl;
    ...
    for ( int i = num_of_sinks; i < num_of_nodes; i++ ) {
        ...
        f_el << "id" << i << endl;
        f_el << "el_e: " << el_e << " er_e: " << er_e << " el: " << el
             << " er: " << er << endl;
        f_el << "nbl: " << node[l].nb << " nbr: " << node[r].nb
             << " gl: " << node[l].g << " gr: " << node[r].g << endl;
        f_el << "l_cl: " << node[l].cl_v << " r_cl: " << node[r].cl_v
             << "    l_t: " << node[l].t << " r_t: " << node[r].t << endl;
        f_el << endl;
        if ( node[r].cl_v < 0 || node[l].cl_v < 0 )
            cout << "id(cl): " << node[l].parent << " l_cl: " << node[l].cl_v
                 << " r_cl: " << node[r].cl_v << endl;
        if ( node[r].t < 0 || node[l].t < 0 )
            cout << "id(t): " << node[l].parent << " l_t: " << node[l].t
                 << " r_t: " << node[r].t << endl;
        if ( el <= 0 || er <= 0 )
            cout << "id(el/er): " << node[l].parent << " el: " << el
                 << " er: " << er << endl;
        //*************************************************************
        ...
        double mlt[4];
        mlt[0] = MD(trr[i].core.l_x, trr[i].core.l_y, trr[l].core.l_x,
        trr[l].core.l_y);
        mlt[1] = MD(trr[i].core.l_x, trr[i].core.l_y, trr[l].core.r_x,
        trr[l].core.r_y);
        mlt[2] = MD(trr[i].core.r_x, trr[i].core.r_y, trr[l].core.l_x,
        trr[l].core.l_y);
        mlt[3] = MD(trr[i].core.r_x, trr[i].core.r_y, trr[l].core.r_x,
        trr[l].core.r_y);
        for (int ii=0; ii<4; ii++){
            if (mlt[ii] < mlml)    mlml = mlt[ii];
        }
        if ( mlml != el_i )    cout << "mlml ERROR: "<< i <<"
        mlml="<<mlml<<endl;
        //if (i == 399 )    cout << "mlml: "<< mlml << "  el_i: "<< el_i<< endl;

        double mrmr = MAX_VALUE;
        double mrt[4];
        mrt[0] = MD(trr[i].core.l_x, trr[i].core.l_y, trr[r].core.l_x,
        trr[r].core.l_y);
        mrt[1] = MD(trr[i].core.l_x, trr[i].core.l_y, trr[r].core.r_x,
        trr[r].core.r_y);
        mrt[2] = MD(trr[i].core.r_x, trr[i].core.r_y, trr[r].core.l_x,
        trr[r].core.l_y);
        mrt[3] = MD(trr[i].core.r_x, trr[i].core.r_y, trr[r].core.r_x,
        trr[r].core.r_y);
        for (int ii=0; ii<4; ii++){
            if (mrt[ii] < mrmr)    mrmr = mrt[ii];
        }
        if ( mrmr != er_i )    cout << "mrmr ERROR: "<< i<<" mrmr=" <<
        mrmr<<endl;
        //if (i == 235 )    cout << "mrmr: "<< mrmr << "  er_i: "<< er_i<< endl;
    */
        // cout<< "id: " << i<< " ms: " << trr[i].core.slope << " " << endl;
        // cout<< trr[i].core.l_x << " " << trr[i].core.l_y << " " <<
        // trr[i].core.r_x << " " << trr[i].core.r_y << endl;
        updateInfo( l, r );
    }
    //*********** debug for trr ****************
    ...
    for ( int i = 0; i < num_of_nodes; i++ ) {
        f_trr << "id=" << i << "  parent=" << node[i].parent
              << "    trr.r=" << trr[i].r << " trr.core=" << trr[i].core.slope
              << " " << trr[i].core.l_x << " " << trr[i].core.l_y << " "
              << trr[i].core.r_x << " " << trr[i].core.r_y << endl;
        if ( trr[i].core.slope != 0 && trr[i].core.slope != 1 )
            cout << "slope error: " << i << endl;
    }
    ...
    for ( int i = num_of_nodes - 1; i > num_of_sinks - 1; i-- ) {
        if ( i == num_of_nodes - 1 ) {
            ...
            par = node[i].parent;
            ev  = trr[i].r;
            // 4 conditions. a terminal point in the square, both two points
            // outside the square
        ...
        // if (i == 221)    cout << "loc_x: "<< loc[i].x << "  loc_y: " <<
        // loc[i].y << endl;
    }
...
void CGBDME::outputRes() {
    ...
    for ( it_sgw = sgw.begin(); it_sgw != sgw.end(); it_sgw++ ) {
        if ( it_sgw == sgw.end() - 1 )
            f_res << 0 << " " << ( *it_sgw ).to + 1 << " 0" << endl;
        else
            f_res << ( *it_sgw ).from + 1 << " " << ( *it_sgw ).to + 1 << " "
                  << WLIB << endl;
    }
    f_res << "num buffer " << num_of_sgb << endl;
    for ( it_sgb = sgb.begin(); it_sgb != sgb.end(); it_sgb++ ) {
        f_res << ( *it_sgb ).from + 1 << " " << ( *it_sgb ).to + 1 << " 0"
              << endl;
    }
...
void CGBDME::readBufLut( int v_sp ) {
    ...
    if ( v_sp >= 1 && v_sp <= 6 )
        VDD_sourcePhase = v_sp;
    else
        cout << "error: VDD and source phase incorrect!" << endl;
    ...
    for ( int i = 0; i < num_of_lut; i++ ) f_lut >> buf >> cl_llmt_lut[i];
    f_lut.close();
}
...
void CGBDME::calcEdgeLength( int l, int r ) {
    ...
    while ( 1 ) {
        ...
        if ( pre_nbl == 0 )
            ...
            el_t = el - (pre_nbl)*max_length - node[l].lmt;
        if ( pre_nbr == 0 )
            er_t = er - node[r].lmt;
        else
            er_t = er - (pre_nbr)*max_length - node[r].lmt;
        // debug
        if ( el_t <= 0 )
            nbl = 0;
        else {
    ...
    if ( node[l].parent == DEBUG_NODE )
        cout << el_i << " " << er_i << " " << node[l].nb << node[r].nb << endl;
    node[l].edge = el_i;
    node[r].edge = er_i;
...
MS CGBDME::mergeMS( int l, int r ) {
    ...
    */
    // if (node[l].parent == 191)    cout << "@@@ 191 :  "<< trr[l].r << "  " <<
    // trr[r].r << " " << trr[l].core.slope << " "<< trr[r].core.slope<< endl;
    // 2. calc l0~l3, r0~r3
    if ( trr[l].core.slope == 1 ) {
        l_ms[0].slope = 0;
        l_ms[0].l_x   = trr[l].core.l_x - el_i;
        l_ms[0].l_y   = trr[l].core.l_y;
        l_ms[0].r_x   = trr[l].core.l_x;
        l_ms[0].r_y   = trr[l].core.l_y - el_i;
        // if (node[l].parent == 399 )  cout << "trr_l: " << trr[l].core.l_x <<
        // "  " << trr[l].core.l_y <<" "<< trr[l].core.r_x << " " <<
        // trr[l].core.r_y << endl;
        if ( node[l].parent == DEBUG_NODE )
            cout << "l[0]: " << endl
                 << l_ms[0].l_x << "  " << l_ms[0].l_y << endl
                 << l_ms[0].r_x << " " << l_ms[0].r_y << endl;
        l_ms[1].slope = 0;
        l_ms[1].l_x   = trr[l].core.r_x;
        l_ms[1].l_y   = trr[l].core.r_y + el_i;
        l_ms[1].r_x   = trr[l].core.r_x + el_i;
        l_ms[1].r_y   = trr[l].core.r_y;
        if ( node[l].parent == DEBUG_NODE )
            cout << "l[1]: " << endl
                 << l_ms[1].l_x << "  " << l_ms[1].l_y << endl
                 << l_ms[1].r_x << " " << l_ms[1].r_y << endl;

        l_ms[2].slope = 1;
        l_ms[2].l_x   = trr[l].core.l_x;
        l_ms[2].l_y   = trr[l].core.l_y - el_i;
        l_ms[2].r_x   = trr[l].core.r_x + el_i;
        l_ms[2].r_y   = trr[l].core.r_y;

        if ( node[l].parent == DEBUG_NODE )
            cout << "l[2]: " << endl
                 << l_ms[2].l_x << "  " << l_ms[2].l_y << endl
                 << l_ms[2].r_x << " " << l_ms[2].r_y << endl;
        l_ms[3].slope = 1;
        l_ms[3].l_x   = trr[l].core.l_x - el_i;
        l_ms[3].l_y   = trr[l].core.l_y;
        l_ms[3].r_x   = trr[l].core.r_x;
        l_ms[3].r_y   = trr[l].core.r_y + el_i;
        if ( node[l].parent == DEBUG_NODE )
            cout << "l[3]: " << endl
                 << l_ms[3].l_x << "  " << l_ms[3].l_y << endl
                 << l_ms[3].r_x << " " << l_ms[3].r_y << endl;
    } else {
        l_ms[2].slope = 1;
        l_ms[2].l_x   = trr[l].core.r_x;
        l_ms[2].l_y   = trr[l].core.r_y - el_i;
        l_ms[2].r_x   = trr[l].core.r_x + el_i;
        l_ms[2].r_y   = trr[l].core.r_y;
        if ( node[l].parent == DEBUG_NODE )
            cout << "l[2]: " << endl
        ...
        l_ms[3].l_x   = trr[l].core.l_x - el_i;
        l_ms[3].l_y   = trr[l].core.l_y;
        l_ms[3].r_x   = trr[l].core.l_x;
        l_ms[3].r_y   = trr[l].core.l_y + el_i;
        if ( node[l].parent == DEBUG_NODE )
            cout << "l[3]: " << endl
                 << l_ms[3].l_x << "  " << l_ms[3].l_y << endl
                 << l_ms[3].r_x << " " << l_ms[3].r_y << endl;
        ...
        l_ms[0].l_x   = trr[l].core.l_x - el_i;
        l_ms[0].l_y   = trr[l].core.l_y;
        l_ms[0].r_x   = trr[l].core.r_x;
        l_ms[0].r_y   = trr[l].core.r_y - el_i;
        if ( node[l].parent == DEBUG_NODE )
            cout << "l[0]: " << endl
                 << l_ms[0].l_x << "  " << l_ms[0].l_y << endl
                 << l_ms[0].r_x << " " << l_ms[0].r_y << endl;
        ...
        l_ms[1].slope = 0;
        l_ms[1].l_x   = trr[l].core.l_x;
        l_ms[1].l_y   = trr[l].core.l_y + el_i;
        l_ms[1].r_x   = trr[l].core.r_x + el_i;
        l_ms[1].r_y   = trr[l].core.r_y;
        if ( node[l].parent == DEBUG_NODE )
            cout << "l[1]: " << endl
    ...
    if ( trr[r].core.slope == 1 ) {
        r_ms[0].slope = 0;
        r_ms[0].l_x   = trr[r].core.l_x - er_i;
        r_ms[0].l_y   = trr[r].core.l_y;
        r_ms[0].r_x   = trr[r].core.l_x;
        r_ms[0].r_y   = trr[r].core.l_y - er_i;
        if ( node[l].parent == DEBUG_NODE )
            cout << "r[0]: " << endl
                 << r_ms[0].l_x << "  " << r_ms[0].l_y << endl
                 << r_ms[0].r_x << " " << r_ms[0].r_y << endl;

        r_ms[1].slope = 0;
        r_ms[1].l_x   = trr[r].core.r_x;
        r_ms[1].l_y   = trr[r].core.r_y + er_i;
        r_ms[1].r_x   = trr[r].core.r_x + er_i;
        r_ms[1].r_y   = trr[r].core.r_y;
        if ( node[l].parent == DEBUG_NODE )
            cout << "r[1]: " << endl
                 << r_ms[1].l_x << "  " << r_ms[1].l_y << endl
                 << r_ms[1].r_x << " " << r_ms[1].r_y << endl;

        r_ms[2].slope = 1;
        r_ms[2].l_x   = trr[r].core.l_x;
        r_ms[2].l_y   = trr[r].core.l_y - er_i;
        r_ms[2].r_x   = trr[r].core.r_x + er_i;
        r_ms[2].r_y   = trr[r].core.r_y;
        if ( node[l].parent == DEBUG_NODE )
            cout << "r[2]: " << endl
                 << r_ms[2].l_x << "  " << r_ms[2].l_y << endl
                 << r_ms[2].r_x << " " << r_ms[2].r_y << endl;

        r_ms[3].slope = 1;
        r_ms[3].l_x   = trr[r].core.l_x - er_i;
        r_ms[3].l_y   = trr[r].core.l_y;
        r_ms[3].r_x   = trr[r].core.r_x;
        r_ms[3].r_y   = trr[r].core.r_y + er_i;
        if ( node[l].parent == DEBUG_NODE )
            cout << "r[3]: " << endl
                 << r_ms[3].l_x << "  " << r_ms[3].l_y << endl
                 << r_ms[3].r_x << " " << r_ms[3].r_y << endl;
    } else {
        r_ms[2].slope = 1;
        r_ms[2].l_x   = trr[r].core.r_x;
        r_ms[2].l_y   = trr[r].core.r_y - er_i;
        r_ms[2].r_x   = trr[r].core.r_x + er_i;
        r_ms[2].r_y   = trr[r].core.r_y;
        if ( node[l].parent == DEBUG_NODE )
            cout << "r[2]: " << endl
        ...
        r_ms[3].l_x   = trr[r].core.l_x - er_i;
        r_ms[3].l_y   = trr[r].core.l_y;
        r_ms[3].r_x   = trr[r].core.l_x;
        r_ms[3].r_y   = trr[r].core.l_y + er_i;
        if ( node[l].parent == DEBUG_NODE )
            cout << "r[3]: " << endl
                 << r_ms[3].l_x << "  " << r_ms[3].l_y << endl
                 << r_ms[3].r_x << " " << r_ms[3].r_y << endl;
        ...
        r_ms[0].l_x   = trr[r].core.l_x - er_i;
        r_ms[0].l_y   = trr[r].core.l_y;
        r_ms[0].r_x   = trr[r].core.r_x;
        r_ms[0].r_y   = trr[r].core.r_y - er_i;
        if ( node[l].parent == DEBUG_NODE )
            cout << "r[0]: " << endl
                 << r_ms[0].l_x << "  " << r_ms[0].l_y << endl
                 << r_ms[0].r_x << " " << r_ms[0].r_y << endl;
        ...
        r_ms[1].slope = 0;
        r_ms[1].l_x   = trr[r].core.l_x;
        r_ms[1].l_y   = trr[r].core.l_y + er_i;
        r_ms[1].r_x   = trr[r].core.r_x + er_i;
        r_ms[1].r_y   = trr[r].core.r_y;
        if ( node[l].parent == DEBUG_NODE )
            cout << "r[1]: " << endl
    ...
    if ( trr[l].r == 0 ) {
        if ( trr[l].core.slope == 0 ) {
            for ( int i = 0; i < 2; i++ ) {
                if ( isColine( trr[l].core.l_x, trr[l].core.l_y, r_ms[i].l_x,
                               r_ms[i].l_y, r_ms[i].r_x, r_ms[i].r_y ) ) {
                    flag2 = 1;
                    if ( trr[l].core.l_x <=
                         r_ms[i].l_x ) {  // l_ms is in the left of r_ms
            ...
            int i_min, i_max;
            if ( trr[l].core.l_x == trr[l].core.r_x &&
                 trr[l].core.l_y == trr[l].core.r_y ) {
                i_min = 0;
                i_max = 4;
            } else {
                i_min = 2;
            ...
            for ( int i = i_min; i < i_max; i++ ) {
                if ( isColine( trr[l].core.l_x, trr[l].core.l_y,
                               trr[l].core.r_x, trr[l].core.r_y, r_ms[i].l_x,
                               r_ms[i].l_y ) ) {
                    flag2 = 1;
        ...
        if ( flag2 == 0 ) {
            ...
            int    index;
            double mlt[4];
            mlt[0] = MD( trr[l].core.l_x, trr[l].core.l_y, trr[r].core.l_x,
                         trr[r].core.l_y );
            mlt[1] = MD( trr[l].core.l_x, trr[l].core.l_y, trr[r].core.r_x,
                         trr[r].core.r_y );
            mlt[2] = MD( trr[l].core.r_x, trr[l].core.r_y, trr[r].core.l_x,
                         trr[r].core.l_y );
            mlt[3] = MD( trr[l].core.r_x, trr[l].core.r_y, trr[r].core.r_x,
                         trr[r].core.r_y );
            for ( int i = 0; i < 4; i++ ) {
                if ( mlt[i] < mlml ) {
                    mlml  = mlt[i];
        ...
        if ( flag == 0 ) ms = ms_t;
        if ( ms.l_x < ms.r_x && ms.l_y < ms.r_y )
            ms.slope = 1;
        else
            ms.slope = 0;
        return ms;
        ...
        if ( trr[r].core.slope == 0 ) {
            for ( int i = 0; i < 2; i++ ) {
                if ( isColine( trr[r].core.l_x, trr[r].core.l_y, l_ms[i].l_x,
                               l_ms[i].l_y, l_ms[i].r_x, l_ms[i].r_y ) ) {
                    flag2 = 1;
                    if ( trr[r].core.l_x <=
                         l_ms[i].l_x ) {  // l_ms is in the left of l_ms
            ...
            int i_min, i_max;
            if ( trr[r].core.l_x == trr[r].core.r_x &&
                 trr[r].core.l_y == trr[r].core.r_y ) {
                i_min = 0;
                i_max = 4;
            } else {
                i_min = 2;
            ...
            for ( int i = i_min; i < i_max; i++ ) {
                if ( isColine( trr[r].core.l_x, trr[r].core.l_y,
                               trr[r].core.r_x, trr[r].core.r_y, l_ms[i].l_x,
                               l_ms[i].l_y ) ) {
                    flag2 = 1;
        ...
        if ( flag2 == 0 ) {
            ...
            int    index;
            double mrt[4];
            mrt[0] = MD( trr[r].core.l_x, trr[r].core.l_y, trr[l].core.l_x,
                         trr[l].core.l_y );
            mrt[1] = MD( trr[r].core.l_x, trr[r].core.l_y, trr[l].core.r_x,
                         trr[l].core.r_y );
            mrt[2] = MD( trr[r].core.r_x, trr[r].core.r_y, trr[l].core.l_x,
                         trr[l].core.l_y );
            mrt[3] = MD( trr[r].core.r_x, trr[r].core.r_y, trr[l].core.r_x,
                         trr[l].core.r_y );
            for ( int i = 0; i < 4; i++ ) {
                if ( mrt[i] < mrmr ) {
                    mrmr  = mrt[i];
    ...
    for ( int i = 0; i < 2; i++ ) {
        for ( int j = 0; j < 2; j++ ) {
            if ( isColine( l_ms[i].l_x, l_ms[i].l_y, l_ms[i].r_x, l_ms[i].r_y,
                           r_ms[j].l_x, r_ms[j].l_y ) ) {
                // if (node[r].parent == 191)  cout << "### i= "<< i << " j="
                // <<j <<endl;
                // ms.slope = 0; ms_t.slope = 0;
                if ( l_ms[i].l_x <=
                     r_ms[j].l_x ) {  // l_ms is in the left of r_ms
                    if ( l_ms[i].r_x > r_ms[j].l_x ) {
                        // if (node[l].parent == 585)    cout <<
                        // "yyyyyyyyyyyyyyyy-----------1"<< endl;
                        flag = 1;
                        if ( l_ms[i].r_x > r_ms[j].r_x ) {
                            // if (node[l].parent == 585)    cout <<
                            // "yyyyyyyyyyyyyyyy-----------2"<< endl;
                            ms.l_x = r_ms[j].l_x;
                            ms.l_y = r_ms[j].l_y;
                            ms.r_x = r_ms[j].r_x;
                            ms.r_y = r_ms[j].r_y;
    ...
    for ( int i = 2; i < 4; i++ ) {
        for ( int j = 2; j < 4; j++ ) {
            if ( isColine( l_ms[i].l_x, l_ms[i].l_y, l_ms[i].r_x, l_ms[i].r_y,
                           r_ms[j].l_x, r_ms[j].l_y ) ) {
                // if (node[r].parent == 191)  cout << "@@@@@@@@@@@@@@@@@ i= "<<
                // i << " j=" << j <<endl;
                // ms.slope = 1; ms_t.slope = 1;
                if ( l_ms[i].l_x <= r_ms[j].l_x ) {
                    if ( l_ms[i].r_x > r_ms[j].l_x ) {
                        if ( node[l].parent == 585 )
                            cout << "yyyyyyyyyyyyyyyy-----------1" << endl;
                        flag = 1;
                        if ( l_ms[i].r_x > r_ms[j].r_x ) {
                            ms.l_x = r_ms[j].l_x;
                            ms.l_y = r_ms[j].l_y;
                        ...
                        // if (node[r].parent == 191)  cout << "111111111111111
                        // " <<endl;
                    } else if ( l_ms[i].r_x == r_ms[j].l_x ) {
                        ms_t.l_x = ms_t.r_x = r_ms[j].l_x;
                        ms_t.l_y = ms_t.r_y = r_ms[j].l_y;
                    ...
                    if ( r_ms[j].r_x > l_ms[i].l_x ) {
                        ...
                        // if (node[r].parent == 191)  cout << "2222222222222"
                        // <<endl;
                    } else if ( r_ms[j].r_x == l_ms[i].l_x ) {
                        ms_t.l_x = ms_t.r_x = r_ms[j].r_x;
                        ms_t.l_y = ms_t.r_y = r_ms[j].r_y;
    ...
    if ( ms.l_x < ms.r_x && ms.l_y < ms.r_y )
        ...
        ms.slope = 0;
    // if (node[l].parent == 191)cout << "ms_191:   " << ms.l_x << " " << ms.l_y
    // << "  " << ms.r_x <<" " << ms.r_y << " " <<ms.slope <<endl;
...
void CGBDME::updateInfo( int l, int r ) {
    ...
    */
    ...
    if ( node[l].g == 1 ) {
        ...
        // else if (node[l].nb == 1)    t1 += delay_b(node[l].lmt, node[l].cl_v)
        // + delay_b(el_i-node[l].lmt, cg);
        else
            t1 += delay_b( node[l].lmt, node[l].cl_v ) +
                  ( node[l].nb - 1 ) * delay_b( max_length, cg ) +
                  delay_b( el_i - node[l].lmt - ( node[l].nb - 1 ) * max_length,
                           cg );
    } else {
        if ( node[l].nb == 0 ) {
            lut_ll1  = el_i;
        ...
        // else if (node[l].nb == 1)    cl1 = cg + c0*(el_i - node[l].lmt);
            ...
            t1 += delay_b( node[l].lmt, node[l].cl_v ) +
                  ( node[l].nb - 1 ) * delay_b( max_length, cg ) +
                  delay_w( el_i - node[l].lmt - ( node[l].nb - 1 ) * max_length,
                           cg );
        }
    }
    ...
    if ( node[r].g == 1 ) {
        ...
        lut_clt2 = cg;
        if ( node[r].nb == 0 )
            t2 += delay_b( er_i, node[r].cl_v );
        else
            t2 += delay_b( node[r].lmt, node[r].cl_v ) +
                  ( node[r].nb - 1 ) * delay_b( max_length, cg ) +
                  delay_b( er_i - node[r].lmt - ( node[r].nb - 1 ) * max_length,
                           cg );
    } else {
        if ( node[r].nb == 0 ) {
            lut_ll2  = er_i;
        ...
        // else if (node[r].nb == 1)    cl2 = cg + c0*(er_i - node[r].lmt);
            ...
            cl2 = cg +
                  c0 * ( er_i - node[r].lmt - ( node[r].nb - 1 ) * max_length );
            t2 += delay_b( node[r].lmt, node[r].cl_v ) +
                  ( node[r].nb - 1 ) * delay_b( max_length, cg ) +
                  delay_w( er_i - node[r].lmt - ( node[r].nb - 1 ) * max_length,
                           cg );
        }
    }
    ...
    for ( int i = 0; i < num_of_nodes; i++ ) {
        ...
        if ( i < num_of_sinks ) {
            ...
            for ( int i_v = 0; i_v < node[i].nv; i_v++ ) {
                ...
                if ( ss == 0 )
                    cout << "pl_x:= " << loc[i].x << " pl_y:= " << loc[i].y
                         << endl;
                if ( ss == 0 )
                    cout << "pl_x:= " << loc[0].x << " pl_y:= " << loc[0].y
                         << endl;
                sgv_t.from = pl_t.id;
                sgv_t.to   = ss;
                sgv.push_back( sgv_t );
        ...
        if ( i == num_of_nodes - 1 ) {
            ...
            x2 = loc[node[i].parent].x;
            y2 = loc[node[i].parent].y;
        }
        ...
        if ( e_t < 0 ) {
            ...
            while ( e_t >= 0 ) {
                ...
                b = e_t;
                if ( flag == 0 ) {
                    // x3 = (a-b+y2*y2-y1*y1+x2*x2-x1*x1+(2*k*x1-2*y1)*(y2-y1))
                    // / (2*k*(y2-y1)+2*(x2-x1));
                    // y3 = k*(x3-x1)+y1;
        ...
        if ( nb_t != node[i].nb )
            cout << "# of BUFFERs ERROR: " << i << "    before: " << node[i].nb
                 << "    after: " << nb_t << endl;
        ...
        num_of_sgw++;
        if ( x3 == 0 && y3 == 0 )
            cout << "555 num_of_pl = " << num_of_pl << endl;
        pl_t.id = num_of_pl++;
        if ( i != num_of_nodes - 1 ) {
...
MS CGBDME::isLineCross( int l, int r ) {
    ...
    if ( trr[l].core.slope == trr[r].core.slope ) {
        ...
        if ( isColine( trr[l].core.l_x, trr[l].core.l_y, trr[l].core.r_x,
                       trr[l].core.r_y, trr[r].core.l_x, trr[r].core.l_y ) ) {
            if ( node[l].parent == DEBUG_NODE )
                cout << "is Line Cross-----------1.1" << endl;
            if ( trr[l].core.l_x < trr[r].core.l_x &&
                 trr[r].core.r_x < trr[l].core.r_x ) {
                if ( node[l].parent == DEBUG_NODE )
                    cout << "is Line Cross-----------1.1.1" << endl;
                ms_t = trr[r].core;
            } else if ( trr[r].core.l_x < trr[l].core.l_x &&
                        trr[l].core.r_x < trr[r].core.r_x ) {
                if ( node[l].parent == DEBUG_NODE )
                    cout << "is Line Cross-----------1.1.2" << endl;
                ms_t = trr[l].core;
            }
        }
    } else {
        if ( node[l].parent == DEBUG_NODE )
            cout << "is Line Cross-----------2" << endl;
...
void ::CGBDME::calcInitEL( int l, int r ) {
    ...
    md = MAX_VALUE;
    double mdt[4];
    mdt[0] = MD(trr[l].core.l_x, trr[l].core.l_y, trr[r].core.l_x,
    trr[r].core.l_y);
    mdt[1] = MD(trr[l].core.l_x, trr[l].core.l_y, trr[r].core.r_x,
    trr[r].core.r_y);
    mdt[2] = MD(trr[l].core.r_x, trr[l].core.r_y, trr[r].core.l_x,
    trr[r].core.l_y);
    mdt[3] = MD(trr[l].core.r_x, trr[l].core.r_y, trr[r].core.r_x,
    trr[r].core.r_y);
    if (node[l].parent == DEBUG_NODE)    cout <<"md calc"<< trr[l].core.l_x <<
    endl << trr[l].core.l_y << endl <<trr[l].core.r_x << endl <<trr[l].core.r_y
    << endl<< trr[r].core.l_x << endl<< trr[r].core.l_y << endl<<trr[r].core.r_x
    << endl << trr[r].core.r_y << endl;
    for (int i=0; i<4; i++){
...
double CGBDME::getLimitLength( int edge ) {
    ...
    if ( de == 0 )
        ...
        delta_c = ( ln9 * de - kb ) / rb + lut_ll * c0;
    ...
    // else cl_index = (int) (node[edge].cl_v+ de+PA);
        ...
        cl_index = (int)( node[edge].cl_v + PA + PB * c0 * lut_ll );
...
void CGBDME::selectSC( int l, int r, int flag ) {
    ...
    // SCl_e[1] = cg + (c0*el_s + USCl) * node[l].p + 0.5*(c0 *
    // dist(node[l].parent) + cg+(ceil(dist(node[l].parent)/max_length)*cg)) *
    // node[l].ptr + node[l].SC - USCl;
    SCl_e[1] = cg + ( c0 * el_s + node[l].nb * cg + USCl ) * node[l].p +
               0.5 * ( c0 * dist( node[l].parent ) + cg ) * node[l].ptr +
               node[l].SC - USCl;

    SCr_e[0] = c0 * er_s + node[r].nb * cg + node[r].SC;
    // SCr_e[1] = cg + (c0*er_s + USCr) * node[r].p + 0.5*(c0 *
    // dist(node[r].parent) + cg+(ceil(dist(node[r].parent)/max_length)*cg)) *
    // node[r].ptr + node[r].SC - USCr;
    SCr_e[1] = cg + ( c0 * er_s + node[r].nb * cg + USCr ) * node[r].p +
               0.5 * ( c0 * dist( node[r].parent ) + cg ) * node[r].ptr +
               node[r].SC - USCr;
...
void CGBDME::calcAccurateEdge( int l, int r ) {
    ...
    // if (node[l].parent == 354)    cout << "354:  "<<node[l].nb<<node[r].nb<<
    // " "<<node[l].g << node[r].g << endl;
    if ( node[l].nb == 0 ) {
        c1  = node[l].cl_v;
        t1  = node[l].t;
        lt1 = 0;
        ...
        c1 = cg;
        t1 = node[l].t + ( node[l].nb - 1 ) * delay_b( max_length, cg ) +
             delay_b( node[l].lmt, node[l].cl_v );
    ...
    if ( node[r].nb == 0 ) {
        ...
        c2 = cg;
        t2 = node[r].t + ( node[r].nb - 1 ) * delay_b( max_length, cg ) +
             delay_b( node[r].lmt, node[r].cl_v );
    ...
    if ( node[l].g == 0 && node[r].g == 0 ) {
        ...
        l1 = ( t2 - t1 - rg * cgo - rg * c1 + r0 * L * ( 0.5 * c0 * L + c2 ) ) /
             ( r0 * ( c1 + c2 + c0 * L ) + rg * c0 );
        if ( l1 >= 0 && l1 <= L )
            l2 = L - l1;
        else if ( l1 < 0 ) {
            ...
            node[r].detour =
                root( 0.5 * r0 * c0, r0 * c2, t2 - t1 - rg * cgo - rg * c1 ) -
                L;
            l2 = L;
        } else {
            l2 = 0;
            // l1 = root(0.5*r0*c0, rg*c0+r0*c1, t1+rg*cgo+rg*c1-t2);
            node[l].detour = root( 0.5 * r0 * c0, rg * c0 + r0 * c1,
                                   t1 + rg * cgo + rg * c1 - t2 ) -
                             L;
            l1 = L;
        }
    } else if ( node[l].g == 0 && node[r].g == 1 ) {
        l2 = ( t1 - t2 - rg * cgo - rg * c2 + r0 * L * ( 0.5 * c0 * L + c1 ) ) /
             ( r0 * ( c1 + c2 + c0 * L ) + rg * c0 );
        if ( l2 >= 0 && l2 <= L )
            l1 = L - l2;
        else if ( l2 < 0 ) {
            ...
            node[l].detour =
                root( 0.5 * r0 * c0, r0 * c1, t1 - t2 - rg * cgo - rg * c2 ) -
                L;
            l1 = L;
        } else {
            l1 = 0;
            // l2 = root(0.5*r0*c0, rg*c0+r0*c2, t2+rg*cgo+rg*c2-t1);
            node[r].detour = root( 0.5 * r0 * c0, rg * c0 + r0 * c2,
                                   t2 + rg * cgo + rg * c2 - t1 ) -
                             L;
            l2 = L;
        }
    } else {
        l1 = ( ( t2 + rg * c2 ) - ( t1 + rg * c1 ) +
               r0 * L * ( 0.5 * c0 * L + c2 ) + rg * c0 * L ) /
             ( r0 * ( c1 + c2 + c0 * L ) + 2 * rg * c0 );
        ...
        if ( l1 >= 0 && l1 <= L )
            ...
            node[r].detour = root( 0.5 * r0 * c0, r0 * c2 + rg * c0,
                                   ( t2 + rg * c2 ) - ( t1 + rg * c1 ) ) -
                             L;
            l2 = L;
        } else {
            ...
            node[l].detour = root( 0.5 * r0 * c0, r0 * c1 + rg * c0,
                                   ( t1 + rg * c1 ) - ( t2 + rg * c2 ) ) -
                             L;
            l1 = L;
        }
    }
    // if(node[l].parent == 354 )    cout << "202 lt1: " << lt1 << "   lt2: "<<
    // lt2 << " l1: " << l1 << "   l2: "<< l2 << endl;
    el = l1 + lt1;
    er = l2 + lt2;
}
...
double CGBDME::lineDist( int l, int r ) {
    ...
    if ( min( x1, x2 ) < max( x3, x4 ) &&
         min( x3, x4 ) < max( x1, x2 ) &&  // is retangle cross?
         min( y1, y2 ) < max( y3, y4 ) && min( y3, y4 ) < max( y1, y2 ) ) {
        if ( node[l].parent == DEBUG_NODE )
            cout << "lineCROSS_vertical" << endl;
...
double CGBDME::dist( int edge ) {
    ...
    return sqrt( ( node[edge].x - chip_size.ur_x / 2 ) *
                     ( node[edge].x - chip_size.ur_x / 2 ) +
                 ( node[edge].y - chip_size.ur_y / 2 ) *
                     ( node[edge].y - chip_size.ur_y / 2 ) );
}
...
double CGBDME::delay_b( double length, double c ) {
    return rg * ( c0 * length + c ) + r0 * length * ( 0.5 * c0 * length + c ) +
           rg * cgo;
}
...
double CGBDME::root( double a, double b, double c ) {
    double out;
    if ( b * b - 4 * a * c < 0 )
        cout << "yuyuyuyyuyuyuyuuyuyuyuyuyuyuyuy" << endl;
...
bool CGBDME::isColine( double x1, double y1, double x2, double y2, double x3,
                       double y3 ) {
    double k;
    /*
    if (y3-y1!=0 && y2-y1!=0)
...
bool CGBDME::isIntersection( double x1, double y1, double x2, double y2,
    ...
    if ( min( x1, x2 ) <= max( x3, x4 ) && min( x3, x4 ) <= max( x1, x2 ) &&
        ...
        double a2b1_x = x2 - x3;
        double a2b1_y = y2 - y3;
        if ( xx( a1b1_x, a1b1_y, b2b1_x, b2b1_y ) *
                 xx( b2b1_x, b2b1_y, a2b1_x, a2b1_y ) >=
             0 ) {
            a1b1_y = -a1b1_y;
            a1b1_x = -a1b1_x;
            if ( xx( a1b1_x, a1b1_y, a2a1_x, a2a1_y ) *
                     xx( a2a1_x, a2a1_y, b2a1_x, b2a1_y ) >=
                 0 ) {
                return 1;
            } else
                return 0;
...
void CGBDME::debug_input() {
    ...
    f_d_in << "chip_size: " << chip_size.ll_x << " " << chip_size.ll_y << " "
           << chip_size.ur_x << " " << chip_size.ur_y << endl;
    f_d_in << "r0_c0_cg_cv: " << r0 << " " << c0 << " " << cg << " " << cv
           << endl;
    f_d_in << "num_of_nodes: " << num_of_nodes << endl;
    for ( int i = 267; i < 280; i++ ) {
        f_d_in << "id: " << node[i].id << " " << node[i].x << " " << node[i].y
               << " " << node[i].z << " " << node[i].cl << " " << node[i].p
               << " " << node[i].ptr << " " << node[i].parent << " "
               << node[i].lchild << " " << node[i].rchild << " "
               << node[i].left_TSV << " " << node[i].right_TSV << endl;
    }
}
...
void CGBDME::debug_loc() {
    ...
    cout << "node: 189 " << loc[189].x << " " << loc[189].y << " "
         << node[189].cl_v << endl;
}
